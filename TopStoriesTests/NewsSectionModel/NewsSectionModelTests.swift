...
extension XCTestCase {
    ...
    func newsSectionModel(catType: String) -> NewsSectionModel? {
        guard let items = homeScreenResponse()?.campaigns else { return nil }
    ...
    func homeScreenResponse() -> HomeScreenResponse? {
        do {
            let newsPage: HomeScreenResponse = try TestUtil().get(from: "MockedHomeScreenResponse.json", in: Bundle(for: NewsSectionModelTests.self))
            return newsPage
        } catch {
            return nil
        }
