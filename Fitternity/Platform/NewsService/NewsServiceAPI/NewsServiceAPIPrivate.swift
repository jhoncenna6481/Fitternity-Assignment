...
class NewsServiceAPIPrivate: NewsServiceAPI {
    ...
    func newsPage(for request: NewsRequestParam,
        ...
        let path = "\(request.newsPageId).json"
        ...
        request.response { (data, response) in
            do {
                ...
                let homeScreenResponse = try decoder.decode(HomeScreenResponse.self,
                                                          from: data)
                let responseParam = NewsResponseParam(newsPage: homeScreenResponse,
                                                  error: nil)
