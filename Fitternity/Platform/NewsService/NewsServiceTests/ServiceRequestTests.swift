...
class ServiceRequestTests: XCTestCase {
    ...
    func testServiceRequestFetchVerifyData() {
        let request = ServiceRequest(requestPath: "https://vod.fitn.in/homescreenapi", session: MockURLSession(fileName: "HomeScreenInStudio"))
    ...
    func testServiceRequestFetchVerifyInvalidURLError() {
        let request = ServiceRequest(requestPath: "https://vod.fitn.in/homescreenapi", session: MockURLSession(fileName: "MockedNewsPageErrorResponse"))
        request.response { (data, response) in
            XCTAssertNil(data)
            }.responseError { (error) in
                switch error {
